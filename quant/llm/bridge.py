...
def context_to_prompt(ctx: DecisionContext) -> Tuple[str, str]:
    ...
    user_payload: Dict[str, Any] = {
        ...
        "session_vwap": round(ctx.session_vwap, 2) if getattr(ctx, "session_vwap", 0.0) > 0 else None,
        "prior_poc": round(ctx.prior_poc, 2) if getattr(ctx, "prior_poc", 0.0) > 0 else None,
        "prior_vah": round(ctx.prior_vah, 2) if getattr(ctx, "prior_vah", 0.0) > 0 else None,
        "prior_val": round(ctx.prior_val, 2) if getattr(ctx, "prior_val", 0.0) > 0 else None,
        ...
        "stacked_imbalance_magnitude": getattr(ctx, "stacked_imbalance_magnitude", 0),
        "contested_bubble_zone": getattr(ctx, "contested_bubble_zone", False),
        "drive_number": getattr(ctx, "drive_number", 0),
        "is_second_drive": bool(getattr(ctx, "drive_entry_valid", False)),
        "squeeze_detected": getattr(ctx, "squeeze_detected", False),
        "squeeze_direction": getattr(ctx, "squeeze_direction", ""),
