...
def _build_core_amt_narrative(data: Dict[str, Any]) -> str:
    ...
    if vah > 0 and val > 0 and price > 0:
        ...
        if va_width > 0:
            if price > vah:
                ...
                parts.append(f"⚠️ Price +{price - vah:.2f} ABOVE VAH (+{pct_above:.0f}% of VA width) — breakout zone")
            elif price < val:
                pct_below = ((val - price) / va_width) * 100
                parts.append(f"⚠️ Price {price - val:.2f} BELOW VAL (-{pct_below:.0f}% of VA width) — breakdown zone")
            else:
    ...
    if dev_poc > 0 and (dev_poc != poc or dev_vah != vah):
        parts.append(f"Developing VA: POC {dev_poc:.0f}, VAH {dev_vah:.0f}, VAL {dev_val:.0f}.")
    ...
    if leg_poc > 0:
        ...
        parts.append(f"Impulse leg: POC {leg_poc:.0f}, VAH {leg_vah:.0f}, VAL {leg_val:.0f}.")
    ...
    if lvn_play:
        if isinstance(lvn_play, dict):
            ...
            parts.append(f"🎯 LVN PLAY: {direction} at {level:.0f} ({confluence}). HIGH CONVICTION.")
        else:
    ...
    if bubble_retests:
        for br in bubble_retests:
            ...
            if price_val > 0:
                parts.append(f"BUBBLE RE-TEST: High volume {side} area at {price_val:.0f} being re-tested.")
