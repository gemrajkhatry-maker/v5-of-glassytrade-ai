    ...
    def evaluate_entry(
        ...
        if has_position:
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=0,
                timing="SKIP", reason="Position already open",
                conviction="NONE", setup_type="NONE"
            )
        
        if is_overseer_running:
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=0,
                timing="WAIT", reason="Overseer processing",
                conviction="NONE", setup_type="NONE"
            )
        ...
        if agent_decision is None:
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=0,
                timing="SKIP", reason="No agent decision",
                conviction="NONE", setup_type="NONE"
            )
        ...
        if agent_dir != "FLAT":
            logger.info("COORDINATOR: dir=%s P=%.3f timing=%s id=%s",
                         agent_dir, agent_prob, agent_timing, _id)
        
        if agent_dir == "FLAT":
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=agent_prob,
                timing="SKIP", reason="Agent says no edge",
                conviction="NONE", setup_type="NONE"
            )
        ...
        if agent_prob >= 0.65:
            ...
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=agent_prob,
                timing="SKIP", reason=f"Probability too low (P={agent_prob:.3f} < 0.55)",
                conviction="NONE", setup_type="NONE"
            )
        ...
        if not getattr(session_info, 'allow_entry', False):
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=agent_prob,
                timing="SKIP", reason="Session does not allow entry",
                conviction="NONE", setup_type="NONE"
            )
        ...
        if agent_dir == "SHORT":
            ...
            if not getattr(settings, 'ALLOW_SHORT', True):
                return EntryEvaluation(
                    should_enter=False, direction="FLAT", probability=agent_prob,
                    timing="SKIP", reason="SHORT not allowed",
                    conviction="NONE", setup_type="NONE"
                )
        ...
        if not should_enter:
            return EntryEvaluation(
                should_enter=False, direction="FLAT", probability=agent_prob,
                timing="SKIP", reason=f"Insufficient conviction (P={agent_prob:.3f})",
                conviction=conviction, setup_type="NONE"
            )
