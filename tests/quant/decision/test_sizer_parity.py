...
from app.domain.fabio_ai.services.position_sizer import (
    PositionSizer as LegacyPositionSizer,
    PositionSize as LegacyPositionSize,
)
...
def test_calculate_valid_parity():
    legacy = LegacyPositionSizer.calculate(100_000.0, 100.0, 99.0, 10.0)
    new = PositionSizer.calculate(100_000.0, 100.0, 99.0, 10.0)
    assert legacy == new


def test_calculate_zero_equity_parity():
    legacy = LegacyPositionSizer.calculate(0.0, 100.0, 99.0, 10.0)
    new = PositionSizer.calculate(0.0, 100.0, 99.0, 10.0)
    assert legacy == new


def test_calculate_equal_entry_sl_parity():
    legacy = LegacyPositionSizer.calculate(100_000.0, 100.0, 100.0, 10.0)
...
def test_calculate_custom_risk_pct_parity():
    ...
    assert_parity(
        LegacyPositionSizer.calculate, PositionSizer.calculate, **kwargs
    )
...
def test_apply_velocity_scaling_parity():
    for lots, vel in [(10, 0.2), (10, 0.01), (10, 0.05), (0, 0.2)]:
        assert_parity(
            LegacyPositionSizer.apply_velocity_scaling,
            PositionSizer.apply_velocity_scaling,
            lots,
            vel,
        )
