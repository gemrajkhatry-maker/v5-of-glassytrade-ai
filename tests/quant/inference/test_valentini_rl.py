...
Ported from backend/tests/unit/domain/test_valentini_rl.py.
...
The Valentini AMT environment and trainer need the optional ``gymnasium``
dependency. When it is absent the whole module is skipped instead of failing
collection (mirrors the pre-existing backend env where gymnasium is missing).
"""
