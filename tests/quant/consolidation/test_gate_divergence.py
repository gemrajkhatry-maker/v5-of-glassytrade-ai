...
from quant.contracts.enums import MarketState as MS, SetupType
...
def green_verdict(state, bar: Bar, agent_dir: str | None):
    ...
    if passed:
        ...
        signal_dir = sig.type if sig is not None else None
...
def test_signal_sltp_geometry_divergence():
    ...
    green_risk = abs(sig.entry - sig.sl)
...
def test_exit_engine_divergence():
    ...
    gf_pos = GreenfieldPosition(
        order=Order(sig, 1), open_price=sig.entry, open_time=sig.timestamp, size=1
    )
    ...
    prod_pos = ProdPosition(
        ...
        size=Decimal("1"),
        initial_stop=Decimal(str(sig.sl)),
