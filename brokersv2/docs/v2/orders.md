...
| orderType required | enum string | Order Type
            ...
            STOP_LOSS 
            STOP_LOSS_MARKET |
| validity required | enum string | Validity of Order
            DAY
            IOC |
...
| amoTime conditionally required | enum sting | Timing to pump the after market order
            PRE_OPEN
            OPEN
            OPEN_30
            OPEN_60 |
...
| transactionType  required | enum string | The trading side of transaction
                ...
                SELL |
| exchangeSegment  required | enum string |
| productType  required | enum string | Product type
                CNC
                INTRADAY
                MARGIN
                MTF |
...
| triggerPrice  conditionally required | float | Price at which the order is triggered, in case of SL-M & SL-L |
| afterMarketOrder  conditionally required | boolean | Flag for orders placed after market hours |
| amoTime  conditionally required | enum string | Timing to pump the after market order
                PRE_OPEN
                OPEN
                OPEN_30
                OPEN_60 |
