...
Mode routing: `QUANT_EXECUTION_MODE=paper` → `session_event_router._try_execute_quant_decision` (`session_event_router.py:647`) maps the stored quant decision → `EntryCoordinator.execute_signal` against `PaperBrokerAdapter`. `off` → decision never computed/executed (byte-identical legacy path).
