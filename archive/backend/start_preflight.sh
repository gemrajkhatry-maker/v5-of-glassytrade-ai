...
# The backend imports both `app` (under backend/) and repository packages such
# as `quant`, `shared`, and `config`. Resolve them regardless of caller cwd.
export PYTHONPATH="$BACKEND_ROOT:$PROJECT_ROOT${PYTHONPATH:+:$PYTHONPATH}"
...
def validate_runtime_env() -> None:
    ...
    # Tooling: validate the interpreter that the launcher will use, rather than
    # relying on a system `uvicorn` executable being on PATH.
    try:
        import importlib.util
        if importlib.util.find_spec("uvicorn") is None:
            add_error("runtime", f"uvicorn is not importable by {sys.executable}")
    except Exception as exc:
        add_error("runtime", f"Unable to validate uvicorn: {exc}")
