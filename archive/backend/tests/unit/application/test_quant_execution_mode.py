...
def _approved_long_signal():
    """Canonical Signal the bridge stores for the approved LONG decision."""
...
def test_router_off_never_executes(monkeypatch):
    ...
    session.last_quant_signal = _approved_long_signal()

    assert router._try_execute_quant_decision(
        "SYM", session, current_bar_time="bar-1"
    ) is False
...
def test_router_shadow_logs_but_does_not_execute(monkeypatch, caplog):
    ...
    with caplog.at_level(
        ...
        executed = router._try_execute_quant_decision(
            "SYM", session, current_bar_time="bar-1"
        )

    assert executed is False  # shadow is observational; legacy path continues
    entry_coordinator.execute_signal.assert_not_called()
    assert any("SHADOW quant execution would execute" in r.message for r in caplog.records)


def test_router_rejects_stale_bar_decision_in_paper(monkeypatch):
    monkeypatch.setattr(SettingsAdapter, "QUANT_EXECUTION_MODE", "paper")
    entry_coordinator = Mock()
    router = _build_router(entry_coordinator, Mock())
    session = SessionState(symbol="SYM")
    decision = _approved_long_decision()
    decision["barTime"] = "bar-1"
    session.last_quant_decision = decision
    session.last_quant_signal = _approved_long_signal()

    assert router._try_execute_quant_decision(
        "SYM", session, current_bar_time="bar-2"
    ) is False
    entry_coordinator.execute_signal.assert_not_called()


def test_router_routes_matching_bar_decision_in_paper(monkeypatch):
    monkeypatch.setattr(SettingsAdapter, "QUANT_EXECUTION_MODE", "paper")
    entry_coordinator = Mock()
    router = _build_router(entry_coordinator, Mock())
    session = SessionState(symbol="SYM")
...
def test_router_paper_and_live_route_to_execute_signal(monkeypatch, mode):
    monkeypatch.setattr(SettingsAdapter, "QUANT_EXECUTION_MODE", mode)
    monkeypatch.setattr(SettingsAdapter, "LIVE_EXECUTION_APPROVED", mode == "live")
    entry_coordinator = Mock()
    router = _build_router(entry_coordinator, Mock())
    session = SessionState(symbol="SYM")
    session.last_quant_decision = _approved_long_decision()
