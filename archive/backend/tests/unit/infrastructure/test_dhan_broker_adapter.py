...
def test_close_sync_delegates_to_underlying_broker():
    """Shutdown must release the Dhan HTTP/WebSocket resources."""
    broker = MagicMock()
    adapter = _make_adapter(broker)

    adapter.close_sync()

    broker.close_sync.assert_called_once_with()


def test_close_sync_is_safe_without_underlying_broker():
    adapter = DhanBrokerAdapter.__new__(DhanBrokerAdapter)
