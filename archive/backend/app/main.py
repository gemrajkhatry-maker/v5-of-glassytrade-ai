...
def close_runtime_transports(app) -> None:
    """Close market-data and execution transports at the sync boundary."""
    for resource_name in ("market_data", "broker"):
        resource = getattr(app.state, resource_name, None)
        close_sync = getattr(resource, "close_sync", None)
        if callable(close_sync):
            try:
                close_sync()
                logger.info("%s closed", resource_name.replace("_", " ").title())
            except Exception:
                logger.debug("%s teardown failed — non-critical", resource_name, exc_info=True)
...
def create_application() -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator:
        ...
        # Flush session fallbacks and stop handler thread pools while storage
        # is still available.
        try:
            app.state.trading_session.cleanup()
            logger.info("Session fallbacks and handler pools cleaned up")
        except Exception:
            logger.debug("Session cleanup failed — non-critical", exc_info=True)

        # Close broker transports before releasing storage. Live Dhan uses
        # aiohttp/WebSocket resources behind a synchronous adapter boundary;
        # leaving this to garbage collection can emit unclosed-session warnings.
