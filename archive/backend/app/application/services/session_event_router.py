...
class SessionEventRouter:
    ...
    def _is_critical_amt_inflection(
        ...
        """
        # Domain prices may be Decimal while AMT values are floats. Normalize
        # before arithmetic so the production tick path is type-stable.
        live_price = float(live_price)
        ib_high = float(amt_result.ib_high or 0.0)
        ib_low = float(amt_result.ib_low or 0.0)
        vwap_dev = float(amt_result.vwap_deviation_sigmas or 0.0)
    ...
    def execute_entry_path(
        ...
        """Execute entry path: quant gate, legacy gates, signal build, persist.
        ...
        if run_entry and _can_execute:
            ...
            # Quant decision engine of record: paper/live may route only an
            # approved decision produced for this exact event bar. Shadow logs
            # the comparison and returns False so the legacy path continues.
            if self._try_execute_quant_decision(
                event.symbol,
                session,
                current_bar_time=current_bar_time or event.tick.time,
            ):
                return
            ...
            if gate_passed:
                ...
                if signal:
                    ...
                    if _tick_trace_id:
                        _meta["tick_trace_id"] = _tick_trace_id
                    # Carry the originating stream context into downstream
                    # decision and position records for replay and repair.
                    for _field in (
                        "session_id",
                        "sequence",
                        "correlation_id",
                        ...
                        _value = getattr(event, _field, None)
    ...
    def _try_execute_quant_decision(
        self,
        symbol: str,
        session: Any,
        current_bar_time: str | None = None,
        ...
        Executes the canonical quant ``Signal`` stored on the session (alongside
        the broadcast DTO) via ``EntryCoordinator``. Returns True when a
        paper/live quant signal is executed so the legacy gate path is skipped;
        shadow returns False so the legacy path continues.

        Mode gate (``QUANT_EXECUTION_MODE``):
        - ``off``: never execute (byte-identical to the old flag-off path).
        - ``shadow``: log what would have been executed, then continue through
          the legacy path for side-by-side comparison; do NOT touch the entry
          coordinator from the quant signal.
        - ``paper``/``live``: map and execute via ``EntryCoordinator``.
        """
        mode = settings.QUANT_EXECUTION_MODE
        if mode == "live" and not settings.LIVE_EXECUTION_APPROVED:
            log.critical(
                "LIVE quant execution blocked for %s: LIVE_EXECUTION_APPROVED is not set",
                symbol,
            )
            return False
        if mode == "off":
        ...
        if not decision or not decision.get("approved"):
            return False
        if mode in ("paper", "live") and not current_bar_time:
            log.warning("Rejecting unbound quant decision for %s", symbol)
            return False
        if current_bar_time and decision.get("barTime") != current_bar_time:
            log.warning(
                "Ignoring stale quant decision for %s: decision_bar=%s current_bar=%s",
                symbol,
                decision.get("barTime"),
                current_bar_time,
            )
            return False
        # The bridge stores the canonical Signal alongside the DTO; execute it
        # directly instead of re-mapping a lossy DTO at this seam.
        signal = getattr(session, "last_quant_signal", None)
        if signal is None:
            return False
        ...
        if mode == "shadow":
            log.info(
                "SHADOW quant execution would execute %s dir=%s entry=%.4f (not routed; legacy continues)",
                symbol,
                direction,
                entry,
            )
