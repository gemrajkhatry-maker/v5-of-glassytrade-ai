...
class SessionState:
    ...
    symbol: str = ""
    # Runtime-unique identity for event correlation. The trading day remains a
    # separate domain concept and is carried by session reset logic.
    ...
    # Canonical quant Signal for the approved decision (executed directly by
    # the router; the dict above is the broadcast/WS DTO).
    last_quant_signal: Signal | None = None
...
class SessionStateManager:
    ...
    def _maybe_reset_symbol_state(
        ...
        """Reset symbol state on a detected new session day.

        Returns ``True`` only when an existing day marker differs from the
        incoming day. Initializing a missing marker is not a rollover.
        ...
        previous_days = (
            ...
            session._explainability_day,
        )
        rollover = any(day and day != day_key for day in previous_days)
