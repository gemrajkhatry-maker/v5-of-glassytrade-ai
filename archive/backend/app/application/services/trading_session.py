...
from app.domain.ops.self_healing import OrderRejectionHandler, get_shared_fallback_buffer
...
class TradingSessionService:
    ...
    def __init__(
        ...
        # The fallback is created before EntryCoordinator so broker-successful
        # positions can be durably queued if storage is unavailable.
        # Restore pending persistence retries from SQLite when the adapter
        # supports the durable outbox.  The in-memory queue remains the
        # immediate fallback if the database itself is unavailable.
        self._db_fallback = get_shared_fallback_buffer(storage)
        # Keep quant state scoped to this backend session; a module-global bridge
        # would leak coordinator/bar-dedup state across restarts and test sessions.
        self._quant_bridge = QuantBridge()
    ...
    def _maybe_reset_symbol_state(
        ...
        if self._state_manager._maybe_reset_symbol_state(session, symbol, tick_time):
            self._quant_bridge.reset(symbol)
            # AMT source selection is session-scoped. Reset it alongside the
            # other per-symbol engines so the next session can choose a fresh
            # underlying/option source without changing the current session's
            # historical meaning.
            self._amt_service.reset(symbol)
    ...
    def _save_closed_trade_to_storage(self, pos, symbol: str) -> None:
        ...
        try:
            ...
            log.error(
                "Trade persistence failed; buffered for retry: %s", e, exc_info=True
            )
    ...
    def _publish_tick_event(
        ...
        try:
            ...
            tick_event = TickReceived(
                ...
                correlation_id=tick_trace.trace_id,
                source="market_data",
            )
    ...
    def _evaluate_quant_decision(
        ...
        """Evaluate the quant brain once for the AMT-selected closed bar.

        The caller captures the bar and immutable facts after exit processing
        while holding the session lock. Keeping bridge evaluation outside that
        lock avoids lock inversion with the risk coordinator and bridge.
        """
        if bar is None:
            with session._lock:
                session.last_quant_decision = None
                session.last_quant_signal = None
            return

        try:
            auction_dto = self._quant_bridge.on_bar_close_with_decision(
                event.symbol,
                bar,
                session,
                facts,
            ...
            # Quant is additive until paper evidence exists. A quant failure
            # must not suppress exits or the established legacy entry path.
            log.warning(
                "quant decision evaluation failed for %s; continuing legacy path",
                event.symbol,
                exc_info=True,
            )
    ...
    def _on_tick(self, event: TickReceived) -> None:
        ...
        amt_analysis_valid = amt_result is not None
        ...
        try:
            ...
            if amt_analysis_valid and self._pre_candle_advisor.should_fire(event.symbol, bar_minute):
        ...
        _stage_start = time.monotonic()
        agent_decision = (
            self._event_router.run_micro_agent_pipeline(
                event, amt_result, self._exchange_config
            )
            if amt_analysis_valid
            else None
        )
        ...
        with session._lock:
            ...
            quant_bar = self._amt_service.latest_closed_bar(cache, event.symbol)
            session_info = cache.get_last_session_info()
            source_data = (
                cache.get_underlying_data()
                if cache.has_underlying_data(5)
                else cache.get_option_data()
            )
            quant_facts = {
                "session_open": not bool(getattr(session_info, "force_exit", False)),
                "warmup_complete": len(source_data) >= 15,
                "position_open": has_position,
                "cooldown_remaining_sec": 1 if _in_cooldown else 0,
                "agent_direction": getattr(agent_decision, "direction", None),
                "agent_probability": float(
                    getattr(agent_decision, "probability", 0.0) or 0.0
                ),
                "equity": float(getattr(session.portfolio, "equity", settings.CAPITAL)),
                "risk_per_trade_pct": float(
                    getattr(settings, "RISK_PER_TRADE_PCT", None) or 0.005
                ),
                "tick_size": float(
        ...
        # Risk-manager reads happen outside session._lock to avoid lock
        # inversion. The bridge receives the post-exit portfolio snapshot and
        # the exact source bar retained by AMTService.
        risk_manager = self._risk_coordinator.get_session_risk_manager(event.symbol)
        risk_state = getattr(risk_manager, "daily_state", None)
        quant_facts["risk_halted"] = bool(self._risk_coordinator.is_halted(event.symbol))
        quant_facts["consecutive_losses"] = int(
            getattr(risk_state, "consecutive_losses", 0)
        )

        self._evaluate_quant_decision(
            event,
            session,
            cache,
            bar=quant_bar,
        ...
        if amt_analysis_valid:
            self._event_router.run_overseer_if_needed(
                session, event, amt_result, self._exchange
            )
        ...
        self._event_router.execute_entry_path(
            ...
            _exec_dir,
            _exec_prob,
            run_entry and amt_analysis_valid,
            self._exchange_config,
            self._allow_short,
            self._scalp_enabled,
            current_bar_time=getattr(quant_bar, "time", None),
        )
        ...
        if (
            trading_enabled
            and amt_analysis_valid
            and SessionOrchestrator.should_fire_entry_llm(
    ...
    def flush_persistence(self) -> int:
        """Flush session-owned fallback writes while storage is still open.

        This is the explicit persistence lifecycle boundary used by both the
        FastAPI shutdown path and ``EngineLifecycle``. It is safe to call more
        than once; an empty queue is a no-op.
        """
        if self._storage is None or not self._db_fallback.buffer_size:
            return 0
        flushed = self._db_fallback.try_flush(self._storage)
        if self._db_fallback.buffer_size:
            log.error(
                "DB fallback still contains %d writes after persistence flush (%d flushed)",
                self._db_fallback.buffer_size,
                flushed,
            )
        return flushed

    def cleanup(self) -> None:
        """Flush persistence and shut down handler thread pools.

        Storage is intentionally still open when the application calls this.
        """
