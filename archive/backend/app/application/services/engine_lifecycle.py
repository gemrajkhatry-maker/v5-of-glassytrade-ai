...
class EngineLifecycle:
    ...
    async def shutdown(self) -> None:
        ...
        # The session owns the fallback queue. Keep this boundary explicit for
        # non-FastAPI callers that stop EngineLifecycle directly; main.py may
        # call it again safely through TradingSessionService.cleanup().
        flush_persistence = getattr(self._session_service, "flush_persistence", None)
        if callable(flush_persistence):
            try:
                flushed = flush_persistence()
                logger.info("Trading session persistence flushed (%d writes)", flushed)
            except Exception:
                logger.error("Trading session persistence flush failed", exc_info=True)
