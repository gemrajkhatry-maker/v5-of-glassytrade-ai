...
def quant_decision_to_dto(decision: Any, bar_time: str | None = None) -> dict:
    """Serialize a ``QuantDecision`` to the stored/WS quant-decision DTO."""
    sig = decision.signal
    meta = (sig.metadata or {}) if sig is not None else {}
    return {
        ...
        "signal": (
            {
                "type": "LONG" if sig.is_buy else "SHORT",
                "entry": round(float(sig.price), 4),
                "sl": round(float(sig.stop_loss), 4),
                "tp": round(float(sig.take_profit), 4),
                "rr": round(float(meta.get("quant_rr", 0.0)), 4),
                "confidence": round(float(meta.get("confidence", 0.0)), 4),
            }
            if sig is not None
            else None
        ),
...
class QuantBridge:
    ...
    def on_bar_close_with_decision(
        ...
        configured ``Strategy`` (``QUANT_STRATEGY``, default ``amt``) is
        evaluated against the resulting ``AuctionState``
        and ``session.last_quant_decision`` (a DTO: {approved, reason, phase,
        barTime, timestamp, signal:{type,entry,sl,tp,rr,confidence}|None}) is stored so
        ...
        decision = get_strategy(settings.QUANT_STRATEGY).evaluate(ctx)
        with session._lock:
            session.last_quant_decision = quant_decision_to_dto(decision, bar.time)
            # The canonical Signal is stored alongside the DTO so the router can
            # execute it directly (no lossy DTO -> Signal re-mapping at the seam).
            session.last_quant_signal = decision.signal
        return dto
    ...
    def _build_decision_context(
        ...
        return DecisionContext(
            ...
            consecutive_losses=int(facts.get("consecutive_losses", 0)),
