...
def build_startup_contracts(
    *,
    ...
    fallback = getattr(trading_session, "_db_fallback", None)
    ...
    checks["persistence_durability"] = (
        _error("fallback durability degraded")
        if getattr(fallback, "durability_degraded", False) is True
        else _ok()
    )
