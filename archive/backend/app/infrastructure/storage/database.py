...
-- Crash-safe retry queue for writes that failed in the application layer.
-- Rows are acknowledged only after the original write succeeds.
...
CREATE INDEX IF NOT EXISTS idx_fallback_outbox_created ON fallback_outbox(created_at, id);

CREATE TABLE IF NOT EXISTS fallback_outbox_quarantine (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    source_id INTEGER NOT NULL,
    write_key TEXT NOT NULL,
    payload TEXT NOT NULL,
    error TEXT NOT NULL,
    quarantined_at TEXT DEFAULT (datetime('now', '+5:30 hours'))
);
"""
...
class SQLiteStorageAdapter(IStorage):
    ...
    def _init_db(self) -> None:
        with self._lock:
            ...
            # Retry IDs were added after the original outbox/snapshot tables.
            # Migrate existing file-backed databases before creating indexes.
            for table, column in (
                ("fallback_outbox", "retry_id"),
                ("fallback_outbox", "created_at_epoch"),
                ("performance_snapshots", "retry_id"),
            ):
                columns = {
                    str(row["name"])
            ...
            # Backfill deterministic retry IDs for legacy rows before enforcing
            # uniqueness. Malformed rows are quarantined and removed from the
            # active queue; they must never be replayed as business writes.
            fallback_rows = self._conn.execute(
                "SELECT id, retry_id, write_key, payload FROM fallback_outbox ORDER BY id"
            ).fetchall()
            for row in fallback_rows:
                ...
                try:
                    ...
                    self._conn.execute(
                        "INSERT INTO fallback_outbox_quarantine "
                        "(source_id, write_key, payload, error) VALUES (?, ?, ?, ?)",
                        (source_id, row["write_key"], str(row["payload"]), str(exc)),
                    )
                    self._conn.execute("DELETE FROM fallback_outbox WHERE id = ?", (source_id,))
                    logger.error("Quarantined malformed fallback outbox row %s", source_id)
                    continue
                if not isinstance(payload, dict):
                    self._conn.execute(
                        "INSERT INTO fallback_outbox_quarantine "
                        "(source_id, write_key, payload, error) VALUES (?, ?, ?, ?)",
                        (source_id, row["write_key"], json.dumps(payload, default=str), "payload is not an object"),
                    )
                    self._conn.execute("DELETE FROM fallback_outbox WHERE id = ?", (source_id,))
                    logger.error("Quarantined non-object fallback outbox row %s", source_id)
                    continue
                retry_id = row["retry_id"] or f"legacy-outbox-{source_id}"
                payload.setdefault("__db_fallback_id", retry_id)
                self._conn.execute(
                    "UPDATE fallback_outbox SET retry_id = ?, payload = ? WHERE id = ?",
                    (retry_id, json.dumps(payload, default=str), source_id),
                )
            duplicate_outbox_ids = self._conn.execute(
                "SELECT retry_id FROM fallback_outbox "
                "WHERE retry_id IS NOT NULL AND retry_id <> '' "
                "GROUP BY retry_id HAVING COUNT(*) > 1"
            ).fetchall()
            for row in duplicate_outbox_ids:
                duplicate_rows = self._conn.execute(
                    "SELECT id FROM fallback_outbox WHERE retry_id = ? ORDER BY id",
                    (row["retry_id"],),
                ).fetchall()
                for duplicate in duplicate_rows[1:]:
                    self._conn.execute(
                        "DELETE FROM fallback_outbox WHERE id = ?",
                        (int(duplicate["id"]),),
                    )
            self._conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_fallback_retry_id "
                "ON fallback_outbox(retry_id) WHERE retry_id IS NOT NULL AND retry_id <> ''"
            )
            duplicate_retry_ids = self._conn.execute(
                "SELECT retry_id FROM performance_snapshots "
                "WHERE retry_id IS NOT NULL AND retry_id <> '' "
                "GROUP BY retry_id HAVING COUNT(*) > 1"
            ).fetchall()
            for row in duplicate_retry_ids:
                duplicate_rows = self._conn.execute(
                    "SELECT id FROM performance_snapshots WHERE retry_id = ? ORDER BY id",
                    (row["retry_id"],),
                ).fetchall()
                for duplicate in duplicate_rows[1:]:
                    self._conn.execute(
                        "UPDATE performance_snapshots SET retry_id = NULL WHERE id = ?",
                        (int(duplicate["id"]),),
                    )
            self._conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_perf_retry_id "
                "ON performance_snapshots(retry_id) WHERE retry_id IS NOT NULL AND retry_id <> ''"
            )
            # A closed position is persisted at most once. This makes replay
            # safe when the original commit succeeds but the caller crashes
            # before acknowledging its fallback item.
            try:
                self._conn.execute(
                ...
                self._conn.execute(
                    "CREATE UNIQUE INDEX IF NOT EXISTS idx_trades_position_id "
                    "ON trades(position_id) WHERE position_id <> ''"
                )
            except sqlite3.Error:
                logger.debug("Failed to create trade idempotency index", exc_info=True)
    ...
    def save_trade(
        ...
        self._execute_write(
            "INSERT OR IGNORE INTO trades (position_id, symbol, side, entry_price, exit_price, "
    ...
    def save_performance_snapshot(self, snapshot: dict[str, Any]) -> None:
        with self._lock:
            try:
                retry_id = str(snapshot.get("__db_fallback_id", ""))
                self._conn.execute(
                    "INSERT OR IGNORE INTO performance_snapshots "
                    "(retry_id, symbol, equity, balance, open_pnl, "
    ...
    # ------------------------------------------------------------------
    # Crash-safe fallback outbox
    # ------------------------------------------------------------------

    def fallback_spool_path(self) -> str | None:
        """Return the local spool path used when SQLite cannot enqueue.

        In-memory databases have no restart boundary, so they intentionally do
        not create a filesystem spool. File-backed databases keep the spool
        beside the database to preserve the same operator backup boundary.
        """
        if self._db_path == ":memory:":
            return None
        return f"{self._db_path}.fallback.jsonl"

    def enqueue_fallback_write(
        self,
        write_key: str,
        ...
        *,
        ...
        """Persist an application retry item before acknowledging it in memory."""
        with self._lock:
            effective_retry_id = retry_id or payload.get("__db_fallback_id")
            effective_created_at = (
                created_at_epoch
                if created_at_epoch is not None
                else payload.get("__db_fallback_time", time.time())
            )
            cursor = self._conn.execute(
                "INSERT OR IGNORE INTO fallback_outbox "
                "(retry_id, write_key, payload, created_at_epoch) "
                "VALUES (?, ?, ?, ?)",
                (
                    effective_retry_id,
                    write_key,
    ...
    def load_fallback_writes(self) -> list[dict[str, Any]]:
        """Load pending fallback writes in FIFO order after a process restart."""
        with self._lock:
            rows = self._conn.execute(
                "SELECT id, retry_id, write_key, payload, created_at_epoch, created_at "
                "FROM fallback_outbox ORDER BY COALESCE(created_at_epoch, 0) ASC, id ASC"
            ).fetchall()
            result: list[dict[str, Any]] = []
            for row in rows:
                result.append(
                    {
                        ...
                        or json.loads(row["payload"]).get("__db_fallback_id")
                        or f"legacy-outbox-{int(row['id'])}",
                        "time": row["created_at_epoch"]
                        if row["created_at_epoch"] is not None
                        else row["created_at"],
                    }
                )
            return result

    def delete_fallback_write(self, item_id: int) -> None:
        """Acknowledge a fallback item after its original write succeeds."""
        self._execute_write(
            "DELETE FROM fallback_outbox WHERE id = ?",
            (int(item_id),),
        )

    def delete_fallback_write_by_retry_id(self, retry_id: str) -> None:
        """Remove an orphaned outbox row identified by its stable retry ID."""
        self._execute_write(
            "DELETE FROM fallback_outbox WHERE retry_id = ?",
            (str(retry_id),),
        )
