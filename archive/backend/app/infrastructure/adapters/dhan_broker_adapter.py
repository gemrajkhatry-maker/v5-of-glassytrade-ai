...
class DhanBrokerAdapter(IBroker):
    ...
    def close_sync(self) -> None:
        """Release the underlying Dhan HTTP/WebSocket clients synchronously.

        The adapter is synchronous at the application boundary, so lifecycle
        owners must be able to close it without reaching into ``_broker``.
        Closing is idempotent in ``DhanBroker`` and safe during shutdown.
        """
        broker = self._broker
        if broker is None:
            return
        try:
            ...
            logger.debug("Dhan broker close failed during shutdown", exc_info=True)
