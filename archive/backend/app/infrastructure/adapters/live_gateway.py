...
class LiveGateway:
    ...
    async def _consume(self, symbol: str) -> None:
        ...
        # Market-data adapters may be lazily initialized. Do this once before
        # opening the stream so the gateway contract is deterministic and a
        # failed initialization cannot be mistaken for an empty market.
        ensure_initialized = getattr(self._dhan, "ensure_initialized_sync", None)
        if callable(ensure_initialized):
            try:
                ensure_initialized()
            except Exception:
                logger.exception("LiveGateway adapter initialization failed")
                return
