...
class SettingsAdapter:
    ...
    @property
    def QUANT_STRATEGY(self) -> str:
        """Select the quant decision policy (``quant.strategy`` registry key).

        Default ``amt`` (the triple-A + VA-fade ``DecisionService``). Swap the
        trading logic without touching the engine by registering a strategy and
        setting ``QUANT_STRATEGY=<name>``.
        """
        return os.getenv("QUANT_STRATEGY", "amt").strip().lower() or "amt"
    ...
    @property
    def LIVE_EXECUTION_APPROVED(self) -> bool:
        """Explicit operator opt-in required before live quant routing."""
        return os.getenv("LIVE_EXECUTION_APPROVED", "false").strip().lower() in {
            "1", "true", "yes", "on"
        }
    ...
    def RISK_PER_TRADE_PCT(self) -> float:
        """Get the configured fractional risk budget for one trade.

        Older application services read this uppercase adapter attribute while
        the typed configuration stores it under ``config.risk``.  Keep the
        compatibility alias total and never return ``None``: a missing or
        malformed value must fail closed to the conservative default.
        """
        raw = None
        if self._mode_config is not None:
            raw = getattr(
                getattr(self._mode_config.system_config, "risk", None),
                "risk_per_trade_pct",
                None,
            )
        if raw is None:
            raw = os.getenv("RISK_PER_TRADE_PCT", "0.005")
        try:
            value = float(raw)
        except (TypeError, ValueError):
            value = 0.005
        return value if value > 0 else 0.005
