...
from quant.contracts.enums import SignalType, Source, SetupType as ST
from quant.contracts.constants import MIN_RR_RATIO
from quant.contracts.entities import Signal
from quant.contracts.tick_utils import round_down_to_tick, round_up_to_tick
from quant.decision.gates.confirmation_bundle import compute_atr
from quant.decision.gates.grading import compute_grade_score
from quant.decision.trade_thesis import build_trade_thesis
...
def build_entry_signal(
    ...
    # A structural target that is closer than the configured R:R floor is not
    # an executable signal. Extend the target only in the trade direction so
    # downstream gates and the emitted Signal share the same risk contract.
    risk = abs(px - stop_price)
    reward = (tp_price - px) if is_buy else (px - tp_price)
    minimum_reward = risk * MIN_RR_RATIO
    if risk > 0 and reward < minimum_reward:
        tp_price = px + minimum_reward if is_buy else px - minimum_reward
    ...
    # Rounding can widen risk or reduce reward by one tick. Re-apply the
    # executable R:R contract to the actual prices that will be submitted.
    if risk > 0 and rr < MIN_RR_RATIO:
        if is_buy:
            tp_price = round_up_to_tick(px + risk * MIN_RR_RATIO, tick_size)
        else:
            tp_price = round_down_to_tick(px - risk * MIN_RR_RATIO, tick_size)
        reward = abs(tp_price - px)
        rr = reward / risk if risk > 0 else 0
