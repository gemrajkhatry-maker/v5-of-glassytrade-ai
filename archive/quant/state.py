...
def _position_to_view(position: Position, fill: Fill | None = None, symbol: str = "") -> dict:
    sig = position.order.signal
    dto = {
        "id": position.open_time,
        "symbol": symbol,
        "side": "LONG" if sig.is_buy else "SHORT",
        "source": "AMT",
        "entryPrice": float(position.open_price),
        "size": float(position.size),
        "stopLoss": float(sig.stop_loss),
        "takeProfit": float(sig.take_profit),
...
class StateProjector:
    ...
    def on_event(self, event: Event) -> None:
        ...
        if isinstance(event, BarClosed):
            ...
            s["portfolio"]["positions"].append(
                _position_to_view(event.position, symbol=event.symbol)
            )
        elif isinstance(event, PositionClosed):
            s["portfolio"] = self._portfolio(s["portfolio"])
            fill = event.fill
            self._remove_open(fill.position.open_time, s["portfolio"])
            s["portfolio"]["closedTrades"].append(
                _position_to_view(fill.position, fill, symbol=event.symbol)
            )
