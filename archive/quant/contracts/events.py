...
class DomainEvent:
    ...
    # Event-stream metadata is additive and backward-compatible. ``sequence``
    # is the per-session ordering cursor; the remaining fields make causality
    # explicit when the synchronous bus is replaced by queued handlers.
    ...
    def ordering_sequence(self) -> int:
        """Return the explicit sequence, with legacy events ordered as zero."""
        return max(0, int(self.sequence or 0))
