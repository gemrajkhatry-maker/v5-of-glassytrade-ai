...
def test_no_trade_without_approved_decision():
    ...
    for evt in opens:
        ...
        assert abs(float(signal.price) - float(decision.signal.price)) <= 1e-9
...
def test_ws_contract_carries_auction_and_quant_decision_on_approved_bars():
    ...
    for evt in trace:
        ...
        assert ws["quantDecision"]["signal"]["type"] == (
            "LONG" if evt.decision.signal.is_buy else "SHORT"
        )
        checks += 1
