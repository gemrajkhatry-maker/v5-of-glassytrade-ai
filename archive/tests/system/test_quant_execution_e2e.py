...
def test_quant_decision_drives_execution_when_mode_paper(monkeypatch):
    ...
    executed = router._try_execute_quant_decision(SYMBOL, session, current_bar_time=session.last_quant_decision.get("barTime"))
...
def test_quant_decision_live_shares_paper_code_path(monkeypatch):
    ...
    monkeypatch.setattr(SettingsAdapter, "LIVE_EXECUTION_APPROVED", True)
    bridge = QuantBridge()
    session = SessionState(symbol=SYMBOL)
    ...
    entry_coordinator = Mock()
    router = _build_router(entry_coordinator, Mock())
    assert router._try_execute_quant_decision(SYMBOL, session, current_bar_time=session.last_quant_decision.get("barTime")) is True
    entry_coordinator.execute_signal.assert_called_once()
...
def test_quant_decision_shadow_computes_but_never_executes(monkeypatch, caplog):
    ...
    # ...but the entry coordinator is never touched; legacy path continues.
