...
class TestEventOrdering:
    ...
    def test_explicit_sequence_controls_reconstruction_order(self):
        """Per-stream sequence wins over append order and timestamp ties."""
        store = InMemoryEventStore()
        later = FillReceived(
            event_id="later",
    ...
    def test_sequence_order_is_grouped_by_session(self):
        """A stream cursor is scoped to its session, not globally."""
        store = InMemoryEventStore()
        for session_id, sequence, event_id in (
            ("session-b", 1, "b1"),
            ("session-a", 2, "a2"),
            ("session-a", 1, "a1"),
        ):
        ...
        events = store.get_events(aggregate_id="T1")
        assert [(event.session_id, event.sequence) for event in events] == [
            ("session-a", 1),
            ("session-a", 2),
            ("session-b", 1),
        ]
