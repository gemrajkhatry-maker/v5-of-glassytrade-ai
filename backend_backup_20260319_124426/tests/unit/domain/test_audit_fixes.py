...
# =====================================================================
# 4. ASYNC PERSISTENCE BUS TESTS
# =====================================================================

class _MockStorage:
    """Fake StoragePort for testing AsyncPersistenceBus."""
    ...
    def save_tick(self, symbol, tick_data):
        with self._lock:
            self.calls.append(("save_tick", (symbol, tick_data)))

    def save_trade(self, trade_data):
        with self._lock:
            self.calls.append(("save_trade", (trade_data,)))

    def save_llm_decision(self, data):
        with self._lock:
            self.calls.append(("save_llm_decision", (data,)))

    def save_performance_snapshot(self, data):
        with self._lock:
            self.calls.append(("save_performance_snapshot", (data,)))

    def save_open_position(self, data):
        with self._lock:
            self.calls.append(("save_open_position", (data,)))

    def delete_open_position(self, pos_id):
        with self._lock:
            self.calls.append(("delete_open_position", (pos_id,)))

    def save_session_profile(self, data):
        with self._lock:
            self.calls.append(("save_session_profile", (data,)))
    ...
    def kv_set(self, key, value):
        with self._lock:
            self.calls.append(("kv_set", (key, value)))

    def kv_get(self, key):
        return None


class TestAsyncPersistenceBus:
    """Validates the async persistence bus queues and drains correctly."""

    def test_writes_are_async(self):
        """Writes should return immediately and be processed in background."""
        from app.infrastructure.async_persistence import AsyncPersistenceBus
        mock = _MockStorage()
        bus = AsyncPersistenceBus(mock)
        bus.start()

        # Fire off writes (non-blocking)
        for i in range(10):
            bus.save_tick(f"SYM{i}", {"price": i})
        ...
        assert all(c[0] == "save_tick" for c in mock.calls)

    def test_queue_full_drops_writes(self):
        """When queue is full, excess writes should be dropped gracefully."""
        from app.infrastructure.async_persistence import AsyncPersistenceBus
        mock = _MockStorage()
        bus = AsyncPersistenceBus(mock, max_queue_size=5)
    ...
    def test_read_operations_are_synchronous(self):
        """Read operations should bypass the queue and go directly to storage."""
        from app.infrastructure.async_persistence import AsyncPersistenceBus
        mock = _MockStorage()
        bus = AsyncPersistenceBus(mock)
        # No start() needed — reads must work directly
        result = bus.query_ticks("SYM")
        assert result == []

    def test_stop_flushes_pending(self):
        """Stop should flush all pending writes before exiting."""
        from app.infrastructure.async_persistence import AsyncPersistenceBus
        mock = _MockStorage()
        bus = AsyncPersistenceBus(mock)
        bus.start()
    ...
    def test_batch_processing(self):
        """Multiple writes should be batched together for efficiency."""
        from app.infrastructure.async_persistence import AsyncPersistenceBus
        mock = _MockStorage()
        bus = AsyncPersistenceBus(mock)
        bus.start()

        # Fire 100 writes rapidly
        for i in range(100):
            bus.save_tick(f"SYM", {"i": i})

        _time.sleep(2.0)
        bus.stop()

        # All 100 should have been processed
        assert len(mock.calls) == 100
