...
export const AIAdvisorCard: React.FC<AIAdvisorCardProps> = React.memo(({
    agentDecision,
    quantDecision,
    portfolio,
    collapsible = true,
    ...
    return (
        <div className="p-3 rounded-xl border border-purple-500/25 bg-gradient-to-br from-purple-950/40 via-slate-950/80 to-black/90 backdrop-blur-md shadow-[0_0_25px_rgba(168,85,247,0.12)] space-y-2.5 font-sans overflow-hidden flex flex-col">
            {/* Header: Title + Role Badge + Model Tags + Latency + Collapse Toggle */}
            <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1 flex-wrap ml-auto">
                    ...
                    {modelVersions.timesfm && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[7.5px] font-mono font-semibold bg-cyan-500/10 text-cyan-300 border border-cyan-500/25">
                            <Cpu className="w-2.5 h-2.5" />
                            TimesFM {modelVersions.timesfm}
                        </span>
                    )}
                    {latencyMs !== undefined && latencyMs > 0 && (
                        <span className="px-1.5 py-0.5 rounded text-[7.5px] font-mono text-slate-400 bg-white/5 border border-white/10">
                            {latencyMs}ms
                        </span>
                    )}
                    <span className="px-1.5 py-0.5 rounded text-[7.5px] font-mono font-bold uppercase tracking-wider bg-purple-500/15 text-purple-300 border border-purple-500/25">
                        {source}
                    </span>
                    ...
                    {collapsible && (
                        <button
                            ...
                            className="p-1 hover:bg-white/10 rounded transition-colors text-slate-400 hover:text-white ml-0.5"
            ...
            {!isExpanded && (
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-white/5">
                    <div className="flex items-center gap-1.5 flex-wrap">
                        <div className={`flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-black border uppercase tracking-wider ${actionConfig.badge}`}>
                            {actionConfig.icon}
                            <span>{actionConfig.label}</span>
                        </div>
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-mono bg-white/5 text-slate-300 border border-white/10 uppercase truncate max-w-[140px]">
                    ...
                    <div className="flex items-center gap-1.5 text-[8.5px] font-mono">
                        {meanForecast > 0 && (
                            <span className="font-bold text-slate-200">
                                ₹{meanForecast.toLocaleString('en-IN', { maximumFractionDigits: 1 })}
                            </span>
                        )}
                        <span className={`px-1.5 py-0.5 rounded text-[7.5px] font-mono ${confBadge}`}>
            ...
            {/* Expanded Body (Scrollable if chart container has restricted height) */}
            {isExpanded && (
                <div className="space-y-3 overflow-y-auto custom-scrollbar pr-0.5 max-h-[calc(78vh-3rem)]">
