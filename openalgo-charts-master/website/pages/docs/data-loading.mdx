...
<Callout type="info">
Loading the *same* series repeatedly (a symbol switch and back, a timeframe flip, a reload,
a [linked grid](/docs/chart-linking) opening one instrument in four panes) is a caching
problem rather than a loading one. Wrap the feed with
[`withBarCache`](/docs/bar-cache) and it is answered from memory, without ever serving a
stale forming bar.
</Callout>
