"""Parity and no-replay guards for runtime execution."""
...
def test_runtime_sources_do_not_include_replay_terms() -> None:
    runtime_root = Path(__file__).resolve().parents[3] / "app" / "runtime"
    assert runtime_root.is_dir()

    replay_hits: list[Path] = []
    for path in runtime_root.rglob("*.py"):
        if not path.is_file():
            continue
        text = path.read_text(encoding="utf-8").lower()
        if re.search(r"\breplay\b", text):
            replay_hits.append(path)

    assert not replay_hits
