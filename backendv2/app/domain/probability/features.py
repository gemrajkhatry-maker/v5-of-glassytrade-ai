...
def extract_features(
    ...
    *,
    ...
    session_vwap = _to_float(getattr(amt_result, "session_vwap", 0.0))
    tick_vwap = _to_float(getattr(tick, "vwap", 0.0))
    vwap = session_vwap if session_vwap > 0 else (tick_vwap if tick_vwap > 0 else 0)
    ...
    f["delta_acceleration"] = (
        _to_float(_flow.delta) - _to_float(data[-2].delta) if len(data) >= 2 else 0.0
    )

    # Cumulative delta last 3 bars
    last3 = data[-3:] if len(data) >= 3 else data
    f["cumulative_delta_3bar"] = sum(_to_float(d.delta) for d in last3)
    ...
    if amt_result.lvns:
        nearest_lvn_dist = min(abs(close - _to_float(lvn)) for lvn in amt_result.lvns)
    ...
    if order_book and order_book.bids and order_book.asks:
        ...
        f["bid_ask_spread_bps"] = (_to_float(best_ask) - _to_float(best_bid)) / mid * 10000 if mid > 0 else 0.0

        bid_q1 = order_book.bids[0].quantity
        ask_q1 = order_book.asks[0].quantity
        total_q1 = bid_q1 + ask_q1
        f["book_imbalance_l1"] = (_to_float(bid_q1) - _to_float(ask_q1)) / total_q1 if total_q1 > 0 else 0.0

        bid_q5 = sum(l.quantity for l in order_book.bids[:5])
        ask_q5 = sum(l.quantity for l in order_book.asks[:5])
        total_q5 = bid_q5 + ask_q5
        f["book_imbalance_l5"] = (_to_float(bid_q5) - _to_float(ask_q5)) / total_q5 if total_q5 > 0 else 0.0

        bid_q20 = sum(l.quantity for l in order_book.bids[:20])
        ask_q20 = sum(l.quantity for l in order_book.asks[:20])

        f["bid_depth_total"] = _to_float(bid_q20)
        f["ask_depth_total"] = _to_float(ask_q20)
        f["book_pressure_ratio"] = _to_float(bid_q20) / _to_float(ask_q20) if ask_q20 else 1.0
    ...
    _oi = _to_float(oi)
    _prev_oi = _to_float(prev_oi)
    oi_change = _oi - _prev_oi
    f["oi_change_pct"] = (oi_change / _prev_oi * 100) if _prev_oi > 0 else 0.0
    ...
    f["oi_volume_ratio"] = _to_float(oi) / _to_float(tick.volume) if _to_float(tick.volume) > 0 and _to_float(oi) > 0 else 0.0

    return {name: _to_float(value) for name, value in f.items() if name in set(FEATURE_NAMES) or name in {"book_imbalance_l20"}}
...
def extract_features_from_row(
    ...
    return {name: _to_float(value) for name, value in f.items()}
...
def _atr(data: "list[OHLC]", period: int) -> float:
    if len(data) < period:
        return 0.0
    return sum(_to_float(d.high) - _to_float(d.low) for d in data[-period:]) / period
