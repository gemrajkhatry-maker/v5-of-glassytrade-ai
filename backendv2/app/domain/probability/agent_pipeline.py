...
if TYPE_CHECKING:
    from app.domain.trading.model.value_objects import OHLC, AMTResult, OrderBook
...
def _to_float(value: object) -> float:
    """Cast any numeric-like value to float with a safe default."""
...
def _to_estimate(raw: object, fallback_long: float = 0.5, fallback_short: float = 0.5):
    """Normalize prediction outputs from v1/v2 adapters into a shared shape."""
    if isinstance(raw, dict):
        return SimpleNamespace(
            p_long_target=_to_float(
                raw.get(
                    "p_long_target",
                    raw.get("p_long", raw.get("long_probability", fallback_long)),
                )
            ),
            p_short_target=_to_float(
                raw.get(
                    "p_short_target",
                    raw.get("p_short", raw.get("short_probability", fallback_short)),
                )
            ),
            expected_mfe_long=_to_float(raw.get("expected_mfe_long", 0.0)),
            expected_mfe_short=_to_float(raw.get("expected_mfe_short", 0.0)),
            calibrated=bool(raw.get("calibrated", False)),
        )
    if hasattr(raw, "p_long_target") and hasattr(raw, "p_short_target"):
        return raw
    direction = getattr(raw, "get", None)
    if callable(direction):
        return SimpleNamespace(
            p_long_target=fallback_long,
            p_short_target=fallback_short,
...
def _get_probability_estimate(probability_engine: IProbabilityInference, features: dict[str, float], fallback_long: float = 0.5, fallback_short: float = 0.5):
    raw = None
    if hasattr(probability_engine, "estimate"):
        raw = probability_engine.estimate(features)
    elif hasattr(probability_engine, "predict"):
        raw = probability_engine.predict(features)
    return _to_estimate(raw, fallback_long=fallback_long, fallback_short=fallback_short)
...
def pick_direction(
    ...
    regime: RegimeState,
    playbook: str,
    probability_estimate: object | None = None,
    ...
    est = probability_estimate or _get_probability_estimate(probability_engine, features)
    p_long = _to_float(est.p_long_target)
    p_short = _to_float(est.p_short_target)
...
def assess_timing(
    ...
    bar_range = _to_float(tick.high) - _to_float(tick.low)
    ...
    delta_abs = abs(_to_float(getattr(tick, "delta", 0)))
    ...
    cvd_slope = _to_float(getattr(amt_result, "cvd_slope", 0))
    ...
    if playbook == "return_to_value":
        ...
        if va_width > 0 and abs(_to_float(tick.close) - _to_float(amt_result.poc)) < va_width * 0.25:
            return "SKIP"
        if direction == "LONG" and _to_float(tick.close) > _to_float(amt_result.poc):
            return "WAIT"
        if direction == "SHORT" and _to_float(tick.close) < _to_float(amt_result.poc):
            return "WAIT"
    ...
    if playbook == "probing_breakout":
        ...
        aggression = _to_float(getattr(amt_result, "aggression", 0))
        if aggression < 2.0:
    ...
    if playbook == "return_to_value":
        ...
        if va_width > 0:
            distance_from_edge = min(
                abs(_to_float(tick.close) - _to_float(amt_result.value_area_high)),
                abs(_to_float(tick.close) - _to_float(amt_result.value_area_low)),
            )
...
def run_agent_pipeline(
    ...
    direction_estimate = _get_probability_estimate(probability_engine, features)
    signal = pick_direction(
        features,
        probability_engine,
        regime,
        playbook,
        probability_estimate=direction_estimate,
    ...
    delta_normalized_option = _to_float(getattr(amt_result, "delta_normalized_option", 0.0))
    # norm_delta: -1 to +1 (from tick, also per-symbol)
    _tick_volume = _to_float(tick.volume)
    _tick_delta = _to_float(getattr(tick, "delta", 0))
    norm_delta = _tick_delta / _tick_volume if _tick_volume > 0 else 0

    # Location penalty: price in tail (outside VA) reduces confidence
    if _to_float(amt_result.value_area_high) > 0 and _to_float(amt_result.value_area_low) > 0:
        _price = _to_float(tick.close)
        if _price < _to_float(amt_result.value_area_low):
            # Below VAL — penalize LONG, mild boost SHORT
            _tail_dist = (_to_float(amt_result.value_area_low) - _price) / max(_to_float(amt_result.value_area_low), 1)
        ...
        elif _price > _to_float(amt_result.value_area_high):
            # Above VAH — penalize SHORT, mild boost LONG
            _tail_dist = (_price - _to_float(amt_result.value_area_high)) / max(_to_float(amt_result.value_area_high), 1)
