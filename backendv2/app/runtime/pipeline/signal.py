...
class SignalGeneration:
    ...
    def _build_signal(self, symbol: str, feature: FeatureVector) -> Signal | None:
        try:
            ...
            context_asl = market.session_phase if market else "MORNING"
            if market is not None:
                if market.market_state == "BEARISH" and feature.vwap_lower_2sigma > feature.vwap:
                    if ofi and ofi.ofi < -0.3 and (micro is None or not micro.iceberg_detected):
                        entry = feature.vwap
                        ...
                        return Signal(
                            ...
                            sl=stop, tp=target, rr=rr, confidence=0.55, reason="Microstructure short",
                            source="pipeline"
                        )

            # fallback: evaluate simplified long setup
            if ofi and ofi.ofi > 0.15 and feature.vwap_upper_1sigma >= feature.vwap and (micro is None or micro.depth_available):
                entry = feature.vwap
                ...
                return Signal(
                    ...
                    sl=stop, tp=target, rr=rr, confidence=0.7, reason="Momentum long",
                    source="pipeline"
                )
