...
def _make_tick(close=100.0):
    return OHLC(open=close, high=close + 1, low=close - 1, close=close, volume=1000, delta=0, time="2025-01-01T10:00:00Z", vwap=close)
...
class TestCushionSL:
    def test_risk_sl_pct_tightens_stop(self):
        ...
        sig = build_entry_signal("LONG", tick, amt, ai_result, SetupType.TREND_MODEL, risk_sl_pct=0.005)
    ...
    def test_risk_sl_pct_none_no_change(self):
        ...
        amt = _make_amt()
        ai_result = {"direction": "LONG", "confidence": "HIGH", "rationale": "test"}
        sig_default = build_entry_signal("LONG", tick, amt, ai_result, SetupType.TREND_MODEL)
        sig_none = build_entry_signal("LONG", tick, amt, ai_result, SetupType.TREND_MODEL, risk_sl_pct=None)
    ...
    def test_risk_sl_respects_wider_quant(self):
        ...
        sig_wide = build_entry_signal("LONG", tick, amt, ai_result, SetupType.TREND_MODEL, risk_sl_pct=0.10)
        sig_default = build_entry_signal("LONG", tick, amt, ai_result, SetupType.TREND_MODEL)
