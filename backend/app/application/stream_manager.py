    ...
    async def stream_with_reconnect(self, connect_state: list[float]):
        ...
        async def _depth_worker():
            try:
                ...
                if not hasattr(self._market_data, "stream_depth_20"):
                    logger.warning("StreamManager: stream_depth_20 not supported by adapter")
                    return
                    
                async for depth in self._market_data.stream_depth_20(self._active_symbols):
        ...
        try:
            while self._running:
                ...
                if self._polling_mode:
                    logger.info("Stream: using REST polling fallback (polling_mode=True)")
                    try:
                ...
                try:
                    ...
                    if consecutive_failures >= _MAX_STREAM_RETRIES:
                        logger.error("Stream: failed %d times — giving up", consecutive_failures)
