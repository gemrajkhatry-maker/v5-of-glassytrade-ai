...
class GatePipeline:
    ...
    def evaluate(self, ctx: GateContext) -> GateResult:
        ...
        # HARD GATE 3: Price near POC in BALANCED state (replaces NO_TRADE)
        # In the 2-state model, BALANCED near POC acts as a "no trade" zone
        if ctx.market_state == MarketState.BALANCED and ctx.zone == "AT_POC":
            if not ctx.is_extreme_deviation:
                return self._hard_fail(
                    3, GateReason.FLAT, "Price at POC dead zone (BALANCED state)"
                )
            else:
                logger.info("Responsive Fade: Overriding BALANCED/POC due to extreme σ deviation")
